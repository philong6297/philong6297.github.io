...
**This post focuses on allocation speed,  demonstrates the causes of memory allocation/deallocation sluggishness, and suggests a (comprehensive) technique list on how to speed up critical places**.
The **access speed** will be the topic of a follow-up post.

# Drawbacks of ``malloc`` and ``free`` design

Because ``malloc`` and ``free`` are parts of the C standard[^iso-cpp], C/C++ compiler vendors have implemented them based on The C Standard Library guideline. Undoubtedly, these functions meet all the requirements for general purposes. However, when talking about performance-critical software, they are usually the first thing individual want to replace.

[^iso-cpp]: At the time of writing, C17 standard [@iso-c-17; @iec-c-17] has stated the definitions of ``malloc`` and ``free`` functions:
...
```{r fig.cap="Given the already-allocated green chunks, when the allocator needs to allocate a new chunk, it traverses the list from Free Blocks Start. For speed optimization, the allocator can return the first chunk of the appropriate size (first-fit algorithm). For memory consumption, the allocator can return the chunk whose size most closely matches the requested size by the calling code (best-fit algorithm)."}
knitr::include_graphics("images/allocator-arena.png")
```
...
```{r fig.cap="Memory Fragmentation Illustration. Each block is 16 bytes in size. Green blocks are marked as allocated, and red blocks are available."}
knitr::include_graphics("images/memory-fragmentation-illustration.png")
```
...
For multi-threaded programs (typical for nowadays), **``malloc`` and ``free`` are required to be thread-safe**[^malloc-thread-safe]. The simplest way a C/C++ library vendor can implement them thread-safe is to introduce mutexes to protect the critical section of those functions. However, it comes with a cost: mutex locking/unlocking are expensive operations on multi-processor systems[^mutex-lock-cost]. **Synchronization can waste the speed advantage, even how quickly the allocator can find a memory block of appropriate size**.

[^malloc-thread-safe]: According to the C17 standard [@iso-c-17; @iec-c-17]:
...
[^mutex-lock-cost]: Stackoverflow user "Dummy00001" has explained that the primary overhead of mutexes is from the memory/cache coherency guarantees [@mutex-lock-cost-explain]. In the same article, the user "Carlo Wood" has also benchmarked the number of clocks it takes to lock/unlock mutex on a decent multi-core machine and gave favorable results with "Dummy00001"'s statement[@mutex-lock-cost-benchmark].
...
# Program slowness caused by allocators
...
According to Donald Knuth [@premature-optimization]:
...
## Contiguous containers of pointers
...
However, this solution seems verbose since it can only work for a single type. Fortunately, several C++ polymorphism libraries have been built to resolve real-world problems. A fitting example is [**proxy**](https://github.com/microsoft/proxy) from Microsoft. It is an open-source, cross-platform, single-header library for the purpose of making runtime polymorphism easier to implement and faster. Wang [@ms-proxy] has posted a detailed introduction on how to use the library.
...
**Fortunately, STL data structures accept a user-specified ``Allocator`` as one of the template arguments**. These structures call ``allocate`` to request memory and ``deallocate``  to release the unneeded memory from the ``Allocator``. Users can specify custom ``Allocator`` (by implementing these functions and satisfying the **STL ``Allocator`` requirements**[^stl-allocator-requirements]) for their own needs.

[^stl-allocator-requirements]: A summary of C++20 Standard ``Allocator`` requirements [@iso-cpp-20; @iec-cpp-20] can be found at [cppreference](https://en.cppreference.com/w/cpp/named_req/Allocator#Requirements)


### STL Allocators - Per-Type Allocator

According to [cppreference](https://en.cppreference.com/w/cpp/named_req/Allocator#Stateful_and_stateless_allocators), issued in 01/02/2023, the C++20 Standard [@iso-cpp-20; @iec-cpp-20] has stated that:
...
#### Example of customizing STL allocator

**Arena Allocator**[^arena-allocator] is commonly used for data structures with little changes after creation. Since deallocation is a no-op, destruction can be significantly fast.

[^arena-allocator]: **Region-based memory management**[@region-based-memory-management] is a type of memory management in which each allocated object is assigned to a region. The region (also known as the arena) is a collection of allocated objects that can be efficiently reallocated or deallocated all at once. Like stack allocation, regions facilitate the allocation and deallocation of memory with low overhead. However, they are more flexible, allowing objects to live longer than the stack frame in which they were allocated. In typical implementations, all objects in a region are allocated in a single contiguous range of memory addresses, similar to how stack frames are typically allocated.

This section provides a naive **Arena Allocator** implementation for ``std::map``.

Given the definition of C++20 ``std::map`` [@iso-cpp-20, @iec-cpp-20], issued in 01/02/2023 from [cppreference](https://en.cppreference.com/w/cpp/container/map):
...
- **When ``allocate`` is called, if available, a memory chunk is taken from the cache** (line 20).
  - If ``allocation_cache_`` is empty, additional memory chunks are taken from the ``common_cache_``(line 10). The ``memory_pool`` will move chunks (as much as possible) from the ``common_cache_`` to the ``allocation_cache_``until ``allocation_cache_`` is full.
...
[^libc++-string]: The implementation of **libc++ ``std::string``** can be found at their [repository](https://github.com/llvm/llvm-project/blob/main/libcxx/include/string). Unfortunately, the source code is hard to read and undocumented. Laity [@libcpp-implementation] has posted an overview of their design and optimisation for this implementation
...
- **Pre-allocate memory upfront**: Some programs pre-allocate all the needed memory at the start and then completely dispense with dynamic memory allocation. MISRA coding guidelines even forbids the usage of dynamic memory in the automotive industry:
  ...
  > MISRA C++ 2008 [@misra-cpp-2008], 18-4-1 - Dynamic heap memory allocation shall not be used.

  > MISRA C:2004 [@misra-c-2004], 20.4 - Dynamic heap memory allocation shall not be used.

  > MISRA C:2012 [@misra-c-2012], 21.3 The memory allocation and deallocation functions of <stdlib.h> shall not be used
...
This section describes a solution to speed up programs by using a better system allocator for your needs. Several open-source allocators try to achieve efficient allocation and deallocation. Despite that, there has yet to be any allocator who has taken the holy grail.
...
When one does not specify any configurations,  the Linux programs use **GNU Allocator**, based on [**ptmalloc**](http://www.malloc.de/en/). Apart from it, there are several other open-source allocators commonly used on Linux: [**tcmalloc**](https://github.com/google/tcmalloc) (by Google), [**jemalloc**](https://github.com/jemalloc/jemalloc) (by Facebook), [**mimalloc**](https://github.com/microsoft/mimalloc) (by Microsoft) and [**hoard allocator**](https://github.com/emeryberger/Hoard).
...
"No Bugs" Hare [@it-hare-testing-allocators] has compared several allocators on a test load that tries to simulate a real-world load.
...
## Notes on using allocators in the program
