...
[^mutex-lock-cost]: Stackoverflow user "Dummy00001" has explained the primary overhead of mutexes is from the memory/cache coherency guarantees [@mutex-lock-cost-explain]. In the same article, user "Carlo Wood" has also benchmarked the number of clocks it takes to lock/unlock mutex on a decent multi-core machine and gave favorable results with "Dummy00001"'s answer [@mutex-lock-cost-benchmark]
...
**Know your domain and profile, and benchmark your program to find the slowness root cause**. These sections are the suggestions for "fine-tuning" the applications. They do not provide or propose techniques to rewrite the program more efficiently.
...
However, this solution can only work for a single type. If you need polymorphism, there are several libraries that you can search for. Personally, I will choose [**proxy**](https://github.com/microsoft/proxy). This is an open-source, cross-platform, single-header C++ library, developed by Microsoft team, for the purpose of making runtime polymorphism easier to implement and faster. Wang [@ms-proxy] has posted a detailed introduction on how to use the library.
...
**``std::allocator`` is per-type allocators**: all the data structures of the same type share one instance of the allocator.
...
**Arena Allocator**[^arena-allocator] is commonly used for data structures that have little changes after their creation. Since deallocation is a no-op, destruction can be significantly fast.

[^arena-allocator]: **Region-based memory management**[@region-based-memory-management] (also known as arena) is a type of memory management in which each allocated object is assigned to a region, which is a collection of allocated objects that can be efficiently reallocated or deallocated all at once. Like stack allocation, regions facilitate allocation and deallocation of memory with low overhead; but they are more flexible, allowing objects to live longer than the stack frame in which they were allocated. In typical implementations, all objects in a region are allocated in a single contiguous range of memory addresses, similarly to how stack frames are typically allocated.
...
## Notes on using allocators in your program
